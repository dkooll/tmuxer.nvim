...
M.config = {
  ...
  border = true,
  max_depth = 2,
  parent_highlight = { fg = "#9E8069" }, -- New highlight config
}
...
local function get_non_current_tmux_sessions()
  local sessions_output = vim.fn.systemlist('tmux list-sessions -F "#{?session_attached,1,0} #{session_name}"')
  local sessions = {}
  for _, line in ipairs(sessions_output) do
    local is_current, name = line:match("^(%d)%s+(.+)$")
    if name and is_current == "0" then table.insert(sessions, name) end
  end
  return sessions
end
  ...
  pickers.new(picker_opts, {
    ...
    attach_mappings = function(prompt_bufnr, map)
      ...
      map("i", "<C-d>", function()
        ...
        if #selections > 0 then
          for _, sel in ipairs(selections) do table.insert(sessions_to_kill, sel.value) end
        else
          local selection = action_state.get_selected_entry()
            ...
            on_exit = function()
              if vim.api.nvim_buf_is_valid(prompt_bufnr) then
                ...
                local new_finder = finders.new_table({
                  ...
                  entry_maker = function(entry) return { value = entry, display = entry, ordinal = entry } end
                })
...
-- end
--
-- -- Optimized: Create tmux session and start Neovim in one operation
-- local function create_tmux_session_with_nvim(session_name, project_path, callback)
--   if session_exists(session_name) then
...
--     })
--   else
--     -- Create new session and start Neovim immediately in one command
--     vim.fn.jobstart({
--       "tmux", "new-session", "-ds", session_name,
--       "-c", project_path, M.config.nvim_alias
--     }, {
...
--   end
-- end
-- M.create_tmux_session_with_nvim = create_tmux_session_with_nvim
--
-- -- Async version of switch_tmux_session
-- local function switch_tmux_session(session_name, callback)
--   vim.fn.jobstart({ "tmux", "switch-client", "-t", session_name }, {
--     on_exit = function(_, _)
--       if callback then
...
--       results = projects,
--       entry_maker = function(entry)
--         -- Create a combined display string with project name and parent
