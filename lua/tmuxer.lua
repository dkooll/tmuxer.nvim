...
local function get_session_stats(session_name)
  local windows = vim.fn.systemlist("tmux list-windows -t " .. vim.fn.shellescape(session_name) .. " | wc -l")
  local panes = vim.fn.systemlist("tmux list-panes -t " .. vim.fn.shellescape(session_name) .. " | wc -l")
  return string.format("Windows: %s | Panes: %s", windows[1] or "0", panes[1] or "0")
end
...
function M.tmux_sessions()
  ...
  pickers.new({
    ...
    finder = finders.new_table {
      results = sessions,
      entry_maker = function(entry)
        local stats = get_session_stats(entry)
        return {
          value = entry,
          display = string.format("%-25s %s", entry, stats),
          ordinal = entry,
        }
      end
    },
