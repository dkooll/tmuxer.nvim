...
M.config = {
  nvim_alias = "nvim", -- default; can be set to an alias/command of your choice
  layout_config = {
    height = 15,
    width = 80,
  }
...
--- Instead of creating the session and then sending keys, we now launch nvim directly.
local function run_nvim_in_session(session_name, project_path, callback)
  vim.fn.jobstart({
    "tmux", "new-session", "-ds", session_name, "-c", project_path, M.config.nvim_alias
  }, {
