...
M.config = {
  nvim_cmd = "nvim",
  batch_size = 3  -- Number of sessions to process simultaneously
}
  ...
  vim.fn.jobstart({ "tmux", "new-session", "-ds", session_name, "-c", project_path }, {
    on_exit = function()
      vim.fn.jobstart({ "tmux", "send-keys", "-t", session_name, M.config.nvim_cmd, "Enter" }, {
        on_exit = function()
          cached_sessions[session_name] = true
          if callback then callback() end
        end
      })
    end
...
local function find_git_projects(workspace_path, max_depth)
  ...
  local chunk_size = 50
  for i = 1, #found_paths, chunk_size do
    local chunk_end = math.min(i + chunk_size - 1, #found_paths)
    for j = i, chunk_end do
      local project_path = found_paths[j]
...
function M.open_workspace_popup(workspace, _)
  ...
  -- Update session cache before showing picker
  ...
  pickers.new({}, {
    ...
    finder = finders.new_table {
      ...
      entry_maker = function(entry)
        local column_width = cached_column_width or update_column_width()
    ...
    attach_mappings = function(prompt_bufnr)
      actions.select_default:replace(function()
        ...
        if #selections > 0 then
          -- Multiple selections: process in batches
          local completed = 0
          local total = #selections

          local function process_batch(index)
            if index > #selections then return end

            local batch_end = math.min(index + M.config.batch_size - 1, #selections)
            local batch_completed = 0

            for i = index, batch_end do
              local project = selections[i].value
              local session_name = string.lower(project.name):gsub("[^%w_]", "_")

              -- Skip if session already exists
              if cached_sessions[session_name] then
                completed = completed + 1
                print(string.format("Session already exists (%d/%d): %s", completed, total, session_name))
                batch_completed = batch_completed + 1
                if batch_completed == (batch_end - index + 1) then
          ...
          -- Single selection with immediate switch
...
function M.tmux_sessions()
  ...
  -- Update cache and get sorted sessions
  local sessions = vim.fn.systemlist('tmux list-sessions -F "#{session_name}"')
  table.sort(sessions)
  update_session_cache()
...
function M.setup(opts)
  ...
  -- Set up autocommands
  local group = vim.api.nvim_create_augroup("TmuxerAutocommands", { clear = true })
