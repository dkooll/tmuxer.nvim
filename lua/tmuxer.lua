...
local function run_nvim_in_session(session_name, project_path)
  ...
  -- Explicitly use M.config.nvim_cmd here
  local send_cmd = string.format("tmux send-keys -t %s '%s' Enter", session_name, M.config.nvim_cmd)
  print("Debug - Using nvim command:", M.config.nvim_cmd) -- Debug print
  os.execute(send_cmd)
end
...
function M.setup(opts)
  -- Set config at the module level
  if opts.nvim_cmd then
    M.config.nvim_cmd = opts.nvim_cmd
    print("Tmuxer: Set nvim command to", opts.nvim_cmd) -- Debug print
  end
...
--local function create_and_run_dev_session(session_name, project_path)
  ...
  ---- Then send the nvim-dev command to the session
    ...
    --attach_mappings = function(prompt_bufnr)
      --actions.select_default:replace(function()
        ...
        --if #selections > 0 then
          ...
          --end
        --else
          ---- Single selection: create session, send nvim-dev command, and switch to it
          --local selection = action_state.get_selected_entry()
          --local project = selection.value
          --local session_name = string.lower(project.name):gsub("[^%w_]", "_")
          --create_and_run_dev_session(session_name, project.path)
          --switch_tmux_session(session_name)
          --print("Created and switched to session: " .. session_name .. " with nvim-dev")
        --end
